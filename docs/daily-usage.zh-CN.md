...
5. 安装完成后，**编辑同一个非 Steam 游戏条目**，不要继续启动 `Battle.net-Setup*.exe`，也不要新建一个新的非 Steam 游戏条目
6. 在 Nushell 中获取已安装的 Battle.net 路径：
   ```nushell
   d2r-bnet-steam
   ```
7. 将输出中的 `target` 填入 Steam 属性里的 **Target**，将 `start_in` 填入 **Start In**。两个字段都要保留双引号，因为路径里有空格和 `(x86)`：
   ```text
   Target:
   ...
   ```
8. 在同一个 Steam 条目的 **Launch Options** 中填入已验证的启动项：
   ```text
   LD_PRELOAD= DISABLE_VK_LAYER_VALVE_steam_overlay_1=1 VK_LOADER_LAYERS_DISABLE='*steam_overlay*' gamemoderun %command%
   ```
   Steam UI 里的 **在游戏中启用 Steam 界面** 开关对这个非 Steam Battle.net 条目不可靠；即使界面显示关闭，实际运行时仍可能注入 `gameoverlayrenderer.so` 并允许 `Shift+Tab` 呼出 overlay。上面的环境变量用于硬禁用 Steam overlay，`gamemoderun` 经实测可明显降低 D2R 的 CPU 占用。
9. 通过 Battle.net 安装并启动 D2R

`Battle.net-Setup*.exe` 只用于第一次安装。日常启动必须指向 prefix 里的 `Battle.net.exe`；否则每次点击 Play 都会再次运行安装器，看起来像“重新安装”或“安装后再更新”。Steam 会按非 Steam 游戏条目创建 Proton prefix，新建条目可能分配新的 `compatdata/<ID>`，所以安装完成后优先编辑原条目。

> Steam 为每个非 Steam 游戏创建独立的 Proton prefix，存储在 `~/.local/share/Steam/steamapps/compatdata/<数字ID>/`。

D2R 启动后，尽量最小化 Battle.net 主窗口。Battle.net 的 Chromium/CEF 界面保持可见时，可能在 D2R 之外继续增加 CPU、GPU 和合成器负载。
...
```nushell
glob ($env.HOME + "/.local/share/Steam/steamapps/compatdata/*/pfx/drive_c/Program Files \\(x86\\)/Diablo II Resurrected")
```
...
很多 mod 会附带 Windows 批处理文件（`*.bat`）来生成配置或补丁文件。先进入上面创建或复制的 mod 目录，再用 `d2r-bat` 运行对应的批处理文件：

```nushell
cd $"($d2r)/mods/ProjectDiablo2"
d2r-bat setup.bat
```

`d2r-bat` 会自动找到 D2R 的 Proton prefix 和 `proton-ge-bin` 里的 Wine。如果不进入具体 mod 目录，也可以直接传相对 `mods/` 的路径：

```nushell
d2r-bat ProjectDiablo2/setup.bat
```
