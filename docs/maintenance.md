...
That uses the USTC Nix cache mirror for binary substitution without keeping the official `cache.nixos.org` cache as a fallback. Workstation modules also declare narrow extra Cachix substituters for Anyrun and Hyprland because those fast-moving desktop inputs have useful upstream caches. The local proxy URL is declared in NixOS config:
